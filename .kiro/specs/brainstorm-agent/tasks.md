...
- [x] Create `.claude/commands/brainstorm.md` with the brainstorm agent prompt
- [x] Implement context loading instructions: read `config/constraints.md` and `logs/idea-registry.md`
- [x] Implement generation step: produce 8-10 raw sketches with working title, 2-sentence pitch, target customer, and keywords
- [x] Implement dedup step: compare each sketch against registry entries using keyword overlap, problem similarity, and title similarity
- [x] Implement dedup logging: append duplicate entries to registry with `Status: duplicate` and `Outcome: Deduped against {matched-slug}`
- [x] Implement replacement generation: for each duplicate, generate replacement sketch (up to 3 retries per slot)
- [x] Implement thematic saturation detection: if 3+ sketches match registry, flag warning for run log
- [x] Implement self-critique step: evaluate surviving sketches on constraint alignment and specificity
- [x] Implement selection step: rank sketches, select top 5 (configurable via config), tighten pitches based on critique
- [x] Implement write step: create `pipeline/{slug}/one-pager.md` for each selected idea using the one-pager template format
